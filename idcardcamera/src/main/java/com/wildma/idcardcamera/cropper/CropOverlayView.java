...
//        bottomRight = new Point(maxX - defaultMargin, maxY - defaultMargin);
    ...
    private void drawBackground_4_3(Canvas canvas) {
        Paint paint = new Paint();
        paint.setColor(Color.parseColor("#66000000"));
        paint.setStyle(Paint.Style.FILL);

        Path path = new Path();
        path.moveTo(topLeft.x-100, topLeft.y-100);
        path.lineTo(topRight.x-100, topRight.y-100);
        path.lineTo(bottomRight.x-100, bottomRight.y-100);
        path.lineTo(bottomLeft.x-100, bottomLeft.y-100);
        path.close();
